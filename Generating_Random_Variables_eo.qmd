...
The problem that the inverse transform sampling method solver is as follows:
...
<!-- NOTE: redo these paths to work on any system! -->
[//]: ![](./../../../Desktop/Diff_Distributions_CDF_Inverses.png)

!["Table of Distributions"](https://github.com/mmuratarat/mmuratarat.github.io/blob/master/_posts/images/Screen%20Shot%202019-10-06%20at%2019.45.52.png?raw=true)


|$\mathbfit{Distribution}$| $\mathbfit{CDF}\enspace F(x)$ | Inverse |
|:-----------------------:|------------------------------:|--------:|
|Exponential | $1 - e^{-x/a}$               |$-a ln(u)$|
|Extreme Value|$1 - e^{-e^{\frac{x-a}{b}}}$ |$a+b\enspace ln(ln(u))$|
|Geometric | $1 - (1 - \rho)^{x}$| $\lceil\frac{ln(u)}{ln(1 - \rho)}\rceil$ |
|Logistic | $1-\frac{1}{1+e^{\frac{x-\mu}{b}}}$ | $\mu - b \enspace ln(\frac{1}{\mu}-1)$ |
|Pareto | $1 - x^{-a}$ | $\frac{1}{u^\frac{1}{a}}$ |
